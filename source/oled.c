...
static LPI2C_Type *I2C_base=NULL;
...
static void OLED_Write_Byte(uint8_t data, uint8_t cmd) {
	...
	if (kStatus_Success == LPI2C_MasterStart(I2C_base, I2C_MASTER_SLAVE_ADDR_7BIT, kLPI2C_Write))
	{
		do {
			LPI2C_MasterGetFifoCounts(I2C_base, NULL, &txCount);
		}while (txCount);

		LPI2C_MasterSend(I2C_base, &cmd_buff[0], 2);
			...
			for(int j=0;j<4;j++) {
				for(int n=0;n<8;n++) {
					...
					if(fontSevenSeg32x24[96*disp+(4*i)+j] &  (1 << n))
						OLED_Draw_Point(x+(24*k)+i, y+(8*j)+n, color);
				}
			}
		}
	}
...
void OLED_7segf(uint8_t x, uint8_t y, float data, int8_t digits, int8_t dp, uint8_t color) {
			...
			for(int j=0;j<4;j++) {
				...
				for(int n=0;n<8;n++) {
					...
					if(fontSevenSeg32x24[96*disp+(4*i)+j] &  (1 << n)) {
						...
						OLED_Draw_Point(x+(26*k)+i, y+(8*j)+n, color);
					}
				}
			}
		}
	...
	if(dp) {
		...
		OLED_7cma(x+26*(digits-dp)-3, y, color);
	}
	...
	for(int i=0; i<4 ;i++) {
		...
		for(int n=0; n<4; n++) {
			...
			if(dot[i] &  (1 << n)) {
				...
				OLED_Draw_Point(x+i, y+n, color);
				OLED_Draw_Point(x+i, y+n+8, color);
			}
		}
	}
}
